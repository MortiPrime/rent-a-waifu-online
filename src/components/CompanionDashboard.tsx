...
import { Camera, MessageSquare, DollarSign, Settings, Calendar, Heart, Users, Star } from 'lucide-react';
...
const CompanionDashboard = () => {
  ...
  const getPlanBadge = (plan: string) => {
    const badges = {
      basic: <Badge variant="secondary" className="bg-blue-500/20 text-blue-300 border-blue-500/30">Básico</Badge>,
      premium: <Badge variant="secondary" className="bg-purple-500/20 text-purple-300 border-purple-500/30">Premium</Badge>,
      vip: <Badge variant="secondary" className="bg-yellow-500/20 text-yellow-300 border-yellow-500/30">VIP</Badge>
    };
                ...
                <Card className="bg-white/10 backdrop-blur-md border-white/20">
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      ...
                      <p className="text-white font-semibold">{profile?.stage_name || 'No configurado'}</p>
                    </div>
                    <div>
                    ...
                    {profile?.pricing ? (
                      <>
                        <div className="flex justify-between">
                          ...
                          <span className="text-white font-semibold">${profile.pricing.basic_chat} MXN</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-white/70">Chat Premium:</span>
                          <span className="text-white font-semibold">${profile.pricing.premium_chat} MXN</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-white/70">Video Llamada:</span>
                          <span className="text-white font-semibold">${profile.pricing.video_call} MXN</span>
                        </div>
                      </>
                    ) : (
                      <p className="text-white/70">No hay precios configurados</p>
                    )}
                  </CardContent>
                </Card>
              </div>
